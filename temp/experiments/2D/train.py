    ...
    for epoch in range(config.max_epoch):
        ...
        if rank == 0:
            ...
            save_checkpoint({
                ...
                'best_loss': best_loss,
            }, config, is_best, save_dir=save_dir, filename=f'MambaULight_epoch_{epoch+1}_loss_{loss_all.avg:.4f}.pth.tar')
...
def get_mamba_B_config():
    ...
    config.save_dir = f'./outputs/foundation_mamba_biomed/{time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime())}/'
