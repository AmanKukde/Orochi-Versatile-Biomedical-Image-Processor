    ...
    with torch.no_grad():
        for batch_idx, (source1, target) in enumerate(tqdm(test_loader)):
            ...
            for i in range(source.size(0)):
                ...
                plt.subplot(131)
                plt.imshow(source[i, 0,].cpu().numpy(), cmap='hot', vmin=0, vmax=1)
                plt.title('Source')
                plt.colorbar()
                
                plt.subplot(132)
                plt.imshow(restored[i, 0].cpu().numpy(), cmap='hot', vmin=0, vmax=1)
                plt.title(f'Restored (PSNR: {psnr_val:.2f})')
                plt.colorbar()

                plt.subplot(133)
                plt.imshow(target[i, 0,].cpu().numpy(), cmap='hot', vmin=0, vmax=1)
                plt.title('Target')
                plt.colorbar()
                
                sample_idx = batch_idx * config.batch_size + i
                plt.savefig(os.path.join(vis_dir, f'sample_{sample_idx}__psnr_{psnr_val:.2f}.png'))
                plt.close()
...
def train(rank, world_size, gpu_ids, config, port):
    ...
    train_set = datasets.UnifmirSRDataset2D(config, is_train=True)
    ...
    for epoch in range(start_epoch, config.max_epoch):
        ...
        for idx, data in enumerate(progress_bar):
            try:
                source1, target = [t.to(device) for t in data]
                source = torch.cat([source1, source1], dim=1)
                logits, aux_loss = model(source, target)  
...
def get_orochi_B_config():
    ...
    # config.data_types = ['CCPs', 'F-actin', 'ER', 'Microtubules']  # A list, contains ['CCPs', 'F-actin', 'ER', 'Microtubules']
    config.data_types = ['Microtubules']  # A list, contains ['CCPs', 'F-actin', 'ER', 'Microtubules']

    #path
    config.checkpoint_dir = './checkpoints/2D/SR/checkpoint.pth.tar'
